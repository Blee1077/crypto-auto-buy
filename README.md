...
```bash
tmp$ sam logs -n StockCheckerFunction --stack-name tmp --tail
```
...
```bash
tmp$ pip install -r tests/requirements.txt --user
# unit test
tmp$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
tmp$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
